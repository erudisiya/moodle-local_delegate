...
$table->head = array(
    ...
    get_string('delegator', 'local_delegate'),
...
foreach ($delegaterecords as $key => $delegaterecord) {
    ...
    array('id' => $delegaterecord->id, 'action' => 'delete',
        'courseid' => $courseid));
    $edit = new moodle_url('/local/delegate/edit.php',
        array('id' => $delegaterecord->id, 'action' => 'update',
        'courseid' => $courseid));
    $approve = new moodle_url('/local/delegate/list.php',
        array('id' => $delegaterecord->id, 'action' => 'approve', 'courseid' => $courseid));
    $decline = new moodle_url('/local/delegate/list.php',
     array('id' => $delegaterecord->id, 'action' => 'decline', 'courseid' => $courseid));
    ...
    } else if ($delegaterecord->action == 1) {
        $actionstr = get_string('approved', 'local_delegate');
    } else if ($delegaterecord->action == 2) {
    ...
    $delegatorname = core_user::get_user($delegaterecord->delegator);
    $userprofileurl = $CFG->wwwroot."/user/profile.php?id=".$delegaterecord->delegator;
    $delegatornamestr = html_writer::start_tag('a', array('href' => $userprofileurl));
    $delegatornamestr .= fullname($delegatorname);
    $delegatornamestr .= html_writer::end_tag('a');
...
if (has_capability('local/delegate:create', context_course::instance($courseid))) {
        $tab .= html_writer::start_tag('li', array('class' => 'nav-item'));
        $tab .= html_writer::start_tag('a', array('class' => 'nav-link',
         'title' => get_string('newapplform', 'local_delegate'), 'href' => $form));
